...
DROP POLICY IF EXISTS "monthly_costs_select_policy" ON proposal_monthly_costs;
DROP POLICY IF EXISTS "monthly_costs_insert_policy" ON proposal_monthly_costs;
DROP POLICY IF EXISTS "monthly_costs_update_policy" ON proposal_monthly_costs;
DROP POLICY IF EXISTS "monthly_costs_delete_policy" ON proposal_monthly_costs;

-- Create new policies with unique names
CREATE POLICY "monthly_costs_select_v2"
  ON proposal_monthly_costs
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "monthly_costs_insert_v2"
  ON proposal_monthly_costs
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "monthly_costs_update_v2"
  ON proposal_monthly_costs
  FOR UPDATE
  TO authenticated
  USING (true)
...
CREATE POLICY "monthly_costs_delete_v2"
  ON proposal_monthly_costs
  FOR DELETE
  TO authenticated
  USING (true);
